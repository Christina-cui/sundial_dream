...
public class NewsController extends BaseController {
    ...
    public void list(String title, String newClass, String publishDate, Integer page, Integer rows, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        if (page == null || page == 0) page = 1;
        if (title == null) title = "";
        if (newClass == null) newClass = "";
        if (publishDate == null) publishDate = "";
        if (rows != 0) newsService.setRows(rows);
        List<News> newsList = newsService.queryNews(title, newClass, publishDate, page);
        /*计算总的页数和总的记录数*/
        newsService.queryTotalPageAndRecordNumber(title, newClass, publishDate);
        /*获取到总的页码数目*/
    ...
    public String frontlist(String title, String newClass, String publishDate, Integer currentPage, Model model, HttpServletRequest request) throws Exception {
        if (currentPage == null || currentPage == 0) currentPage = 1;
        if (title == null) title = "";
        if (newClass == null) newClass = "";
        if (publishDate == null) publishDate = "";
        List<News> newsList = newsService.queryNews(title, newClass, publishDate, currentPage);
        /*计算总的页数和总的记录数*/
        newsService.queryTotalPageAndRecordNumber(title, newClass, publishDate);
        /*获取到总的页码数目*/
        int totalPage = newsService.getTotalPage();
    ...
    public void OutToExcel(String title, String newClass, String publishDate, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        if (title == null) title = "";
        if (newClass == null) newClass = "";
        if (publishDate == null) publishDate = "";
        List<News> newsList = newsService.queryNews(title, newClass, publishDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "News信息记录";
        String[] headers = {"新闻id", "新闻标题", "新闻分类", "发布时间"};
        List<String[]> dataset = new ArrayList<String[]>();
        for (int i = 0; i < newsList.size(); i++) {
            News news = newsList.get(i);
            dataset.add(new String[]{news.getNewsId() + "", news.getTitle(), news.getNewClass(), news.getPublishDate()});
        }
        /*
