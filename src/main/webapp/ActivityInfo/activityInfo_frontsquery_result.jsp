...
<%@ page import="com.cuijing.sundial_dream.dto.ActivityInfoVo" %>
<%@ page import="com.cuijing.sundial_dream.entity.ActivityType" %>

<%
    String path = request.getContextPath();
    String basePath = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + path + "/";
    List<ActivityInfoVo> activityInfoList = (List<ActivityInfoVo>) request.getAttribute("activityInfoList");
    //获取所有的typeObj信息
...
<div class="container">
        ...
        <div class="row">
            <%
                ...
                for (int i = 0; i < activityInfoList.size(); i++) {
                    ...
                    ActivityInfoVo activityInfo = activityInfoList.get(i); //获取到公益活动对象
                    String clearLeft = "";
                    if (i % 4 == 0) clearLeft = "style=\"clear:left;\"";
            %>
            <div class="col-md-3 bottom15" <%=clearLeft %>>
                <a href="<%=basePath  %>ActivityInfo/<%=activityInfo.getActivityId() %>/frontshow"><img
                        class="img-responsive" src="<%=basePath%><%=activityInfo.getActivityPhoto()%>"/></a>
                <div class="showFields">
                    <div class="field">
                        活动id:<%=activityInfo.getActivityId() %>
                    </div>
                    <div class="field">
                        活动类型:<%=activityInfo.getType().getTypeName() %>
                    </div>
                    <div class="field">
                    ...
                    <a class="btn btn-primary top5"
                       href="<%=basePath %>ActivityInfo/<%=activityInfo.getActivityId() %>/frontshow">详情</a>
                    <a class="btn btn-primary top5" onclick="activityInfoEdit('<%=activityInfo.getActivityId() %>');"
                       style="display:none;">修改</a>
                    <a class="btn btn-primary top5" onclick="activityInfoDelete('<%=activityInfo.getActivityId() %>');"
            ...
            <div class="row">
                <div class="col-md-12">
                    <nav class="pull-left">
                        <ul class="pagination">
                            <li><a href="#" onclick="GoToPage(<%=currentPage-1 %>,<%=totalPage %>);"
                            ...
                            <li class="<%= currentPage==i?"active":"" %>"><a href="#"
                                                                             onclick="GoToPage(<%=i %>,<%=totalPage %>);"><%=i %>
                            </a></li>
                            <% } %>
                            <li><a href="#" onclick="GoToPage(<%=currentPage+1 %>,<%=totalPage %>);"><span
                                    aria-hidden="true">&raquo;</span></a></li>
                        </ul>
                    </nav>
                    <div class="pull-right" style="line-height:75px;">共有<%=recordNumber %>条记录，当前第 <%=currentPage %>
                        /<%=totalPage %> 页
                    </div>
                </div>
            </div>
    ...
    <div class="col-md-3 wow fadeInRight">
        ...
        <form name="activityInfoQueryForm" id="activityInfoQueryForm" action="<%=basePath %>ActivityInfo/frontlist"
                    ...
                    <%
                        for (ActivityType activityTypeTemp : activityTypeList) {
                            ...
                            if (typeObj != null && typeObj.getTypeId() != null && typeObj.getTypeId().intValue() == activityTypeTemp.getTypeId().intValue())
                                selected = "selected";
                    %>
                    <option value="<%=activityTypeTemp.getTypeId() %>" <%=selected %>><%=activityTypeTemp.getTypeName() %>
                    </option>
                    <%
                        }
                    %>
    ...
    <div class="modal-dialog" style="width:900px;" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body" style="height:450px; overflow: scroll;">
                <form class="form-horizontal" name="activityInfoEditForm" id="activityInfoEditForm"
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-9">
                            <input type="text" id="activityInfo_activityId_edit" name="activityInfo.activityId"
                                   class="form-control" placeholder="请输入活动id" readOnly>
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="activityInfo_typeObj_typeId_edit" class="col-md-3 text-right">活动类型:</label>
                        <div class="col-md-9">
                            <select id="activityInfo_typeObj_typeId_edit" name="activityInfo.typeObj.typeId"
                                    class="form-control">
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-md-9">
                            <input type="text" id="activityInfo_title_edit" name="activityInfo.title"
                                   class="form-control" placeholder="请输入活动主题">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-md-9">
                            ...
                            <input type="hidden" id="activityInfo_activityPhoto_edit" name="activityInfo.activityPhoto"/>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-9">
                            <textarea name="activityInfo.content" id="activityInfo_content_edit"
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-9">
                            <input type="text" id="activityInfo_activityTime_edit" name="activityInfo.activityTime"
                                   class="form-control" placeholder="请输入活动时间">
                        </div>
                    </div>
                </form>
                <style>#activityInfoEditForm .form-group {
                    margin-bottom: 5px;
                }  </style>
            </div>
