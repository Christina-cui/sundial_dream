...
public class DonationController extends BaseController {
    ...
    public String add(Model model, HttpServletRequest request) throws Exception {
        ...
        /*查询所有的UserInfo信息*/
        List<com.cuijing.sundial_dream.entity.User> userInfoList = userService.list();
    ...
    public void list(@ModelAttribute("userObj") User userObj, String dunationTime, String sheHeState, Integer page, Integer rows, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        if (page == null || page == 0) page = 1;
        if (dunationTime == null) dunationTime = "";
        if (sheHeState == null) sheHeState = "";
        Page<Donation> pagea = new Page<>(page,rows);
...
//        List<Donation> donationList = donationService.queryDonation(userObj, dunationTime, sheHeState, page);
        DonationCondition condition = new DonationCondition();
        condition.setUserId(userObj.getId());
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        condition.setDonationGeTime(sdf.parse(dunationTime));
        IPage<Donation> donations = donationService.selectDonationByPage(pagea,condition);
//        /*计算总的页数和总的记录数*/
